...
public class ResourcesAllocationExercise {
	...
	// resources, constraint: states that share the border must not share the same color	
	...
	public static int count(Map<Integer, Integer> result) {
		
		return (int) result.values().stream().filter(p -> p > 0).count();
	}
