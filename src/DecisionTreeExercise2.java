public class DecisionTreeExercise2 {
	 ...
	 * lose $10,000.
	 * 
	 * The probability that a manuscript will succeed is 0.2, and the probability
	 * that it will fail is 0.8.
	 * 
	 * 
	 * Reviewer has the following conditional probability:
	 * 
	 * Let R be a random variable of reviewer's opinion Let S be a random variable
	 * of book would be a success
	 * 
	 * P(S=T) = 0.2 P(S=F) = 0.8
	 * 
	 * P(R=T|S=T) = 0.7, P(R=T and S=T)/P(S=T) = 0.7, P(R=T and S=T) = 0.14
	 * P(R=T|S=F) = 0.4, P(R=T and S=F)/P(S=F) = 0.4, P(R=T and S=F) = 0.32 P(R=T) =
	 * P(R=T and S=T) + P(R=T and S=F) = P(R=T) = 0.46 P(R=F) = 1 - P(R=T) = 0.54
	 * 
	 * Based on P(R=T|S=T) = 0.7 P(S=T|R=T)P(R=T)/P(S=T) = 0.7 P(S=T|R=T) = 0.7 *
	 * P(S=T) / P(R=T) = 0.7 * 0.2 / 0.46 = 0.3043 = 0.30434784
	 * 
	 * Based on P(R=T|S=F) = 0.4 P(S=F|R=T)P(R=T)/P(S=F) = 0.4 P(S=F|R=T) = 0.4 *
	 * P(S=F) / P(R=T) = 0.4 * 0.8 / 0.46 = 0.6956 = 0.69565217
	 * 
	 * Let A1 be the above strategy Exp(Outcome(A1)) = Exp(Outcome(A1)|R=T)P(R=T) +
	 * Exp(Outcome(A1)|R=F)P(R=F)
	 * 
	 * Since Exp(OutCome(A1)|R=F)P(R=F) = 0 Exp(Outcome(A1)) =
	 * Exp(Outcome(A1)|R=T)P(R=T) Exp(Outcome(A1)) = $50,000 * P(S=T|R=T)P(R=T) +
	 * (-10,000) * P(S=F|R=T)P(R=T) Exp(Outcome(A1)) = $50,000 * P(R=T|S=T)P(S=T) +
	 * (-10,000) * P(R=T|S=F)P(S=F) Exp(Outcome(A1)) = $50,000 * 0.7 * 0.2 +
	 * (-10,000) * 0.4 * 0.8 Exp(Outcome(A1)) = $3,800
	 * 
	 * Reviewer's fee is $500
	 * 
	 */
	public static void main(String[] args) {
		// TODO Auto-generated method stub
		DecisionTree.Node<ExpectValue> reviewerDecision = DecisionTree.createDecisionNode(new ExpectValue("Decisions"));
